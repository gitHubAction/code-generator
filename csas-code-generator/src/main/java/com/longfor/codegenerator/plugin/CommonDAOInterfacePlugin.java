...
public class CommonDAOInterfacePlugin extends PluginAdapter {
    ...
    public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
        ...
        if (stringHasValue(daoTargetPackage)) {
            ...
            // 添加公共selectAll接口(自定义的sql在DaoExtPlugin插件中sqlMapDocumentGenerated生产)
            Method selectAllMethod = new Method();
            selectAllMethod.setName("selectAll");
            selectAllMethod.addParameter(new Parameter(new FullyQualifiedJavaType("Model"),"record"));
            selectAllMethod.setReturnType(new FullyQualifiedJavaType("List<Model>"));
            mapperInterface.addMethod(selectAllMethod);
